  ...
  return (
    <AccordionHeaderPrimitive className="flex">
      <AccordionTriggerPrimitive
        className={cn(
          "focus-visible:border-ring focus-visible:ring-ring/50 flex flex-1 items-start justify-between gap-4 rounded-md py-4 text-left text-sm font-medium transition-all outline-none focus-visible:ring-[3px] disabled:pointer-events-none disabled:opacity-50 [&[data-state=open]>span]:rotate-180 [&[data-state=open]>span]:bg-white [&[data-state=open]>span]:border-transparent",
          className,
        )}
        {...props}>
        {children}
        {showArrow && (
          <span className="text-muted-foreground pointer-events-none w-10 h-10 rounded-full border border-border-color flex items-center justify-center shrink-0 translate-y-0.5 transition-transform duration-200">
